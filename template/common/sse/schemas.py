...
class SSEGatewayRequestInfo(BaseModel):
    ...
    headers: dict[str, str] = Field(
        default_factory=dict, description="Client request headers"
    )
...
class SSEGatewayConnectCallback(BaseModel):
    ...
    request: SSEGatewayRequestInfo = Field(
        ..., description="Client request information"
    )
...
class SSEGatewayDisconnectCallback(BaseModel):
    ...
    reason: str = Field(
        ..., description="Disconnect reason (client_closed, server_closed, error)"
    )
    token: str = Field(..., description="Gateway connection token")
    request: SSEGatewayRequestInfo = Field(
        ..., description="Original client request information"
    )
...
class SSEGatewaySendRequest(BaseModel):
    ...
    token: str = Field(..., description="Gateway connection token")
    event: SSEGatewayEventData | None = Field(
        None, description="Event to send (optional if just closing)"
    )
    close: bool = Field(
        default=False, description="Whether to close connection after sending"
    )
